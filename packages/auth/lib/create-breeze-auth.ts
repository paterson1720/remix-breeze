import { SessionStorage, createCookieSessionStorage, json, redirect } from "@remix-run/node";
import {
  ...
  RequireAuthOptions,
  RequireRoleOptions,
  CreateBreezeAuthOptions,
  CreateBreezeAuthWithCustomSessionStorageOptions,
} from "./types";
 ...
 * ```
 */
export function createBreezeAuth<T extends BreezeAuthSessionUser>(
  breezeAuthOptions: CreateBreezeAuthOptions<T> | CreateBreezeAuthWithCustomSessionStorageOptions<T>
) {
  ...
  const providers: BreezeAuthProvider[] = [];
  ...
  return {
     ...
     */
    async registerUser(request: Request, options: { authenticateAndRedirectTo?: string } = {}) {
     ...
     */
    async authenticateWithCredentials(request: Request, options: { redirectTo: string }) {
      const providerConfig = providers.find((provider) => provider.type === "credentials");
